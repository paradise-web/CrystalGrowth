...
def reviewer_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # 调试输出（可选，生产环境可以注释掉）
        # print(f"🔍 [调试] 审核 LLM 原始返回: {review_text[:200]}...")
        ...
        print(f"📊 [Role B] 审核完成: {'通过' if review_result.get('passed') and not has_errors else '发现问题'}")
        ...
        error_msg = str(e)
        print(f"⚠️ [Role B] 审核过程出错: {error_type}: {error_msg}")
        import traceback
...
def _parse_review_result(review_text: str) -> dict:
    ...
    try:
        ...
        print(f"⚠️ [Role B] JSON 解析错误: {e}")
        print(f"   原始文本片段: {review_text[:200]}...")
    except Exception as e:
        print(f"⚠️ [Role B] 解析审核结果时出错: {type(e).__name__}: {e}")
        print(f"   原始文本片段: {review_text[:200]}...")
    
    # 如果解析失败，返回默认结果
    print("⚠️ [Role B] 无法解析审核结果，使用默认值（通过审核）")
    return {
        "passed": True,
        "issues": [],
        "correction_hints": ""
        ...
        for failed in failed_experiments:
            if (current_params["method"] == failed["method"] and 
                ...
                issues.append({
                    "severity": "error",
                    "field": "process",
                    "description": f"⚠️ 检测到与失败实验 #{failed['id']} 相似的配方（方法: {failed['method']}, 温度: {failed['high_temp']}）",
...
def formatter_node(state: AgentState) -> AgentState:
            ...
            if main_compound:
                try:
                    ...
                    if material_info:
                        ...
                        print(f"  ✅ 已补充 {main_compound} 的物理性质: {list(material_info.keys())}")
                    else:
                        print(f"  ℹ️ 未找到 {main_compound} 的物理性质信息")
                except Exception as e:
                    print(f"  ⚠️ [Knowledge Enhancement] 检索失败 ({main_compound}): {e}")
    else:
    ...
    try:
        ...
        for path in style_reference_paths:
            if os.path.exists(path):
                try:
                    ...
                    print(f"  ✅ 已加载风格参考文件: {path}")
                    break
                except Exception as e:
                    print(f"  ⚠️ 读取风格参考文件失败 ({path}): {e}")
        ...
        print("  ✅ 使用 LLM 生成 Markdown 完成")
    except Exception as e:
        print(f"  ⚠️ LLM 生成 Markdown 失败: {e}，回退到传统方法")
        # 回退到传统方法
    ...
    if output_path:
        try:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                f.write(markdown)
            print(f"✅ [Role C] 文件已保存: {output_path}")
            ...
            if base > 0:  # 确保基准值有效
                for v in valid:
                    ...
                    if isinstance(ingredients, dict):
                        ...
                        print(f"  ✅ 已计算摩尔比: {v['item'].get('name', '未知')} = {ratio_str}")
                    else:
                        # 旧格式：直接设置 molar_ratio
                        v['item']['molar_ratio'] = ratio_str
                        print(f"  ✅ 已计算摩尔比: {v['item'].get('compound', '未知')} = {ratio_str}")
...
def generate_markdown_with_llm(
    ...
    for exp in data.get("experiments", []):
        ...
        for p in current_precursors:
            ...
            print(f"  🔍 [Cheat Sheet] {name}: 原始值={repr(molar_ratio_raw)}, 处理后={ratio}")
    ...
    if cheat_sheet_rows:
        ...
        print(f"  ⚠️ [Cheat Sheet] 警告：未找到任何配料数据！")
    # =================================================================
    ...
    if cheat_sheet_rows:
        user_content_parts.extend([
            "**【真实配料表数据（必须使用）】**",
            "⚠️ **最高优先级指令**：以下是预生成的、包含正确摩尔比的配料表数据。",
...
def generate_markdown(data: dict, image_reference_path: str = "", feedback: str = "") -> str:
    ...
    for idx, exp in enumerate(experiments):
        ...
        if isinstance(method_specific, dict):
            ...
            if geometry:
                md_output.append(f"* 🔬 **空间放置**：{geometry}")
...
def human_review_node(state: AgentState) -> AgentState:
    ...
    if human_feedback:
        # 用户已提供反馈，根据反馈决定是否通过
        print(f"✅ 收到人工反馈: {human_feedback}")
        if review_passed_override is not None:
            print(f"📊 审核结果: {'通过' if review_passed_override else '未通过'}")
            return {
                **state,
...
if __name__ == "__main__":
    ...
    if os.path.exists(image_file):
        ...
        if review_issues:
            print(f"\n{'='*70}")
            print(f"⚠️  审核问题汇总 (共 {len(review_issues)} 个)")
            print(f"{'='*70}")
